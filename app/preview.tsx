...
export default function PreviewScreen() {
  ...
  //     const audioBufferedDuration = exactDuration + 0.5;

  //     // FFmpeg command to create video with exact matching audio portion
  //     // const ffmpegCommand = `-f concat -safe 0 -i ${inputListPath} -i ${audioPath} -filter_complex "[1:a]atrim=0:${exactDuration}[aout]" -map 0:v:0 -map "[aout]" -vf "scale=720:1280,format=yuv420p" -r 30 -pix_fmt yuv420p ${outputPath}`;

  //     // console.log("🎥 FFmpeg command:", ffmpegCommand);
  //     const ffmpegCommand = `-f concat -safe 0 -i ${inputListPath} -i ${audioPath} -filter_complex "[1:a]atrim=0:${audioBufferedDuration}[aout]" -map 0:v:0 -map "[aout]" -vf "scale=720:1280,format=yuv420p" -r 30 -pix_fmt yuv420p -shortest ${outputPath}`;
